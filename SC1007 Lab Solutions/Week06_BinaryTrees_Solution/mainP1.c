...
void printSmallerValues(BTNode *node, int K) {
    if (node == NULL) return;
    if (node->item < K) printf("%d ", node->item);
    if (node->left) printSmallerValues(node->left, K);
    if (node->right) printSmallerValues(node->right, K);
}