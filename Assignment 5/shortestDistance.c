...
int SD(Graph g, int v, int z){
    ...
    enqueue(&q, v); 
    // queueing the first vertex
    g.visited[v-1] = 1;
    // marked as visited and will double up as distance tracker
    
    while (isEmptyQueue(q) == 0) {
        nextV = dequeue(&q); 
        // visiting all nextV adjacent vertices
        for (int i=0; i<g.V; i++) { 
            if (g.matrix[nextV-1][i] == 1 && g.visited[i] == 0) {
                g.visited[i] = g.visited[nextV-1] + 1;
                enqueue(&q,i+1);
            }
        }
    }
